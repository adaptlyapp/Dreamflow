...
export const AdaptlyAd = () => {
  ...
  const s1out = interpolate(frame, [scene2Start - 30, scene2Start], [1, 0], { extrapolateLeft: "clamp", extrapolateRight: "clamp", easing: Easing.in(Easing.cubic) });
  ...
  const s2opacity = interpolate(frame, [scene2Start - 5, scene2Start + 20, scene2Start + scene2Duration - 30, scene2Start + scene2Duration], [0, 1, 1, 0], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });
  ...
  const s3opacity = interpolate(frame, [scene3Start - 5, scene3Start + 15, scene3Start + scene3Duration - 30, scene3Start + scene3Duration], [0, 1, 1, 0], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });
  const phoneY = interpolate(frame, [scene3Start + 20, scene3Start + 90], [600, 0], { extrapolateRight: "clamp", easing: Easing.out(Easing.cubic) });
  const phoneRotate = interpolate(frame, [scene3Start + 20, scene3Start + 90], [18, 0], { extrapolateRight: "clamp", easing: Easing.out(Easing.cubic) });
  ...
  const s4opacity = interpolate(frame, [scene4Start, scene4Start + 20, scene4Start + scene4Duration - 30, scene4Start + scene4Duration], [0, 1, 1, 0], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });
  ...
  const s5opacity = interpolate(frame, [scene5Start - 5, scene5Start + 20, scene5Start + scene5Duration - 30, scene5Start + scene5Duration], [0, 1, 1, 0], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });
  ...
  const s6opacity = interpolate(frame, [scene6Start - 5, scene6Start + 20, scene6Start + scene6Duration - 30, scene6Start + scene6Duration], [0, 1, 1, 0], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });
  ...
  const s7opacity = interpolate(frame, [scene7Start - 5, scene7Start + 20, scene7Start + scene7Duration - 30, scene7Start + scene7Duration], [0, 1, 1, 0], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });
  ...
  const s8opacity = interpolate(frame, [scene8Start - 5, scene8Start + 20, scene8Start + scene8Duration - 30, scene8Start + scene8Duration], [0, 1, 1, 0], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });
  ...
  const s9opacity = interpolate(frame, [scene9Start - 5, scene9Start + 20, scene9Start + scene9Duration - 30, scene9Start + scene9Duration], [0, 1, 1, 0], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });
  ...
  const s10opacity = interpolate(frame, [scene10Start - 5, scene10Start + 20, scene10Start + scene10Duration - 30, scene10Start + scene10Duration], [0, 1, 1, 0], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });
  ...
  const s11opacity = interpolate(frame, [scene11Start - 5, scene11Start + 20], [0, 1], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });
  const heartScale11 = spring({ frame: frame - scene11Start, fps, config: { damping: 18, stiffness: 70, mass: 1.3 } });
  const heartbeat = interpolate(frame, [0, 45, 90], [1, 1.08, 1], { extrapolateRight: "wrap" });
  const glowPulse11 = interpolate(frame, [0, 55, 110], [0.5, 1, 0.5], { extrapolateRight: "wrap" });
  const headline11Y = interpolate(frame, [scene11Start + 10, scene11Start + 50], [60, 0], { extrapolateRight: "clamp", easing: Easing.out(Easing.back(1.3)) });
  const headline11Opacity = interpolate(frame, [scene11Start + 10, scene11Start + 50], [0, 1], { extrapolateRight: "clamp" });
  const sub11Y = interpolate(frame, [scene11Start + 30, scene11Start + 65], [40, 0], { extrapolateRight: "clamp", easing: Easing.out(Easing.cubic) });
  const sub11Opacity = interpolate(frame, [scene11Start + 30, scene11Start + 65], [0, 1], { extrapolateRight: "clamp" });
  const cta11Scale = spring({ frame: frame - (scene11Start + 50), fps, config: { damping: 15, stiffness: 140, mass: 0.8 } });
  const cta11Opacity = interpolate(frame, [scene11Start + 50, scene11Start + 75], [0, 1], { extrapolateRight: "clamp" });
  const tag11Opacity = interpolate(frame, [scene11Start + 80, scene11Start + 110], [0, 1], { extrapolateRight: "clamp" });
  ...
  const t10flash = interpolate(frame, [scene11Start - 15, scene11Start - 5, scene11Start + 10], [0, 0.7, 0], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });
  ...
  return (
    <div style={{ flex: 1, backgroundColor: "#060910", backgroundImage: `radial-gradient(ellipse 900px 700px at 20% 30%, rgba(16,185,129,0.14) 0%, transparent 55%), radial-gradient(ellipse 700px 600px at 80% 70%, rgba(59,130,246,0.11) 0%, transparent 55%), radial-gradient(ellipse 600px 500px at 55% 95%, rgba(99,102,241,0.08) 0%, transparent 50%), radial-gradient(ellipse 500px 400px at 90% 10%, rgba(16,185,129,0.06) 0%, transparent 50%)`, position: "relative", overflow: "hidden" }}>
      <div style={{ position: "absolute", inset: -200, background: `conic-gradient(from ${bgRotate}deg at 50% 50%, rgba(16,185,129,0.04), rgba(59,130,246,0.03), rgba(99,102,241,0.02), transparent 60%)`, pointerEvents: "none" }} />
      <svg style={{ position: "absolute", inset: 0, width: "100%", height: "100%", opacity: 0.06, pointerEvents: "none" }}>
        <defs><pattern id="dots" x="0" y="0" width="40" height="40" patternUnits="userSpaceOnUse"><circle cx="1" cy="1" r="1" fill="rgba(16,185,129,0.8)" /></pattern></defs>
        <rect width="100%" height="100%" fill="url(#dots)" />
      </svg>
      <div style={{ position: "absolute", inset: 0, backgroundImage: `url("data:image/svg+xml,%3Csvg viewBox='0 0 512 512' xmlns='http://www.w3.org/2000/svg'%3E%3Cfilter id='n'%3E%3CfeTurbulence type='fractalNoise' baseFrequency='0.75' numOctaves='4'/%3E%3C/filter%3E%3Crect width='100%25' height='100%25' filter='url(%23n)' opacity='0.03'/%3E%3C/svg%3E")`, mixBlendMode: "overlay", opacity: 0.6 }} />

      {([
        { top: "18%", left: "12%", size: 9, color: "16,185,129", offset: p1 }, { top: "62%", right: "18%", size: 13, color: "59,130,246", offset: p2 },
        { top: "33%", right: "10%", size: 6, color: "16,185,129", offset: p3 }, { bottom: "22%", left: "22%", size: 11, color: "99,102,241", offset: p4 },
        { top: "10%", right: "35%", size: 5, color: "59,130,246", offset: p1 }, { top: "75%", left: "8%", size: 7, color: "16,185,129", offset: fp1 },
        { top: "45%", left: "45%", size: 4, color: "251,191,36", offset: fp2 }, { bottom: "10%", right: "12%", size: 8, color: "99,102,241", offset: fp3 },
        { top: "5%", left: "55%", size: 6, color: "16,185,129", offset: fp2 }, { top: "88%", left: "40%", size: 5, color: "59,130,246", offset: p3 },
        { top: "55%", right: "5%", size: 9, color: "16,185,129", offset: fp1 }, { top: "25%", left: "38%", size: 4, color: "251,191,36", offset: p4 },
      ] as Array<{ top?: string; bottom?: string; left?: string; right?: string; size: number; color: string; offset: number }>).map((p, i) => (
        <div key={i} style={{ position: "absolute", top: p.top, left: p.left, right: p.right, bottom: p.bottom, width: p.size, height: p.size, borderRadius: "50%", backgroundColor: `rgba(${p.color},0.55)`, transform: `translateY(${p.offset}px)`, boxShadow: `0 0 ${p.size * 3}px rgba(${p.color},0.8)`, opacity: 0.9 * ambientPulse }} />
      ))}
        ...
        <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center", opacity: s1out, perspective: "1200px" }}>
          <svg style={{ position: "absolute", inset: 0, width: "100%", height: "100%", opacity: helixOpacity }} viewBox="0 0 1920 1080">
            {Array.from({ length: 12 }).map((_, i) => {
              const y = 100 + i * 80; const amp = 60; const freq = 0.012; const shift = helixShift * 6;
              ...
              return (<g key={i}><polyline points={points1} fill="none" stroke="rgba(16,185,129,0.2)" strokeWidth="1.5" /><polyline points={points2} fill="none" stroke="rgba(59,130,246,0.15)" strokeWidth="1.5" /></g>);
            })}
          </svg>
          {[{ size: 500, color: "16,185,129", speed: 1, dot: 12 }, { size: 350, color: "59,130,246", speed: -1.3, dot: 10 }, { size: 650, color: "99,102,241", speed: 0.6, dot: 7 }, { size: 200, color: "251,191,36", speed: 2.1, dot: 6 }].map((r, i) => (
          ...
          <div style={{ fontSize: 32, fontFamily: "'DM Sans',sans-serif", fontWeight: 400, color: "rgba(255,255,255,0.7)", transform: `translateX(${subX}px)`, opacity: subOpacity, textAlign: "center", lineHeight: 1.5 }}>Post-discharge recovery guidance + tracking<br /><span style={{ color: "rgba(255,255,255,0.45)", fontSize: 26 }}>for rehab and care teams</span></div>
          <div style={{ position: "absolute", top: 80, right: 150, transform: `translateY(${badgeY + badgeBob}px)`, opacity: badgeOpacity }}>
            <div style={{ padding: "12px 22px", background: "rgba(16,185,129,0.12)", border: "1px solid rgba(16,185,129,0.4)", borderRadius: 50, display: "flex", alignItems: "center", gap: 10, boxShadow: "0 0 30px rgba(16,185,129,0.2)" }}>
              ...
              <div><div style={{ fontSize: 16, fontWeight: 700, color: "#10b981", fontFamily: "'DM Sans',sans-serif" }}>Best Health App 2024</div><div style={{ fontSize: 12, color: "rgba(255,255,255,0.5)", fontFamily: "'DM Sans',sans-serif" }}>HealthTech Innovation Award</div></div>
            </div>
          </div>
          <div style={{ position: "absolute", bottom: 120, left: 140, transform: `translateY(${badgeBob * -0.7}px)`, opacity: badgeOpacity }}>
            <div style={{ padding: "12px 22px", background: "rgba(59,130,246,0.12)", border: "1px solid rgba(59,130,246,0.4)", borderRadius: 50, display: "flex", alignItems: "center", gap: 10, boxShadow: "0 0 30px rgba(59,130,246,0.2)" }}>
              <div style={{ fontSize: 22 }}>📈</div>
              <div><div style={{ fontSize: 16, fontWeight: 700, color: "#3b82f6", fontFamily: "'DM Sans',sans-serif" }}>10,000+ patients guided</div><div style={{ fontSize: 12, color: "rgba(255,255,255,0.5)", fontFamily: "'DM Sans',sans-serif" }}>across 50+ health systems</div></div>
            </div>
          </div>
        </div>
      )}

      {frame >= scene2Start - 30 && frame < scene2Start + 5 && (<div style={{ position: "absolute", inset: 0, background: "linear-gradient(135deg,#10b981,#3b82f6)", opacity: t1flash }} />)}

      {/* SCENE 2 — PROBLEM: reduced padding+gap, smaller fonts to prevent top cutoff */}
      {frame >= scene2Start && frame < scene3Start && (
        <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center", opacity: s2opacity, padding: "30px 100px", gap: 16 }}>
          <div style={{ position: "relative", display: "flex", justifyContent: "center", alignItems: "center" }}>
            <div style={{ position: "absolute", width: 140, height: 140, borderRadius: "50%", border: "2px solid rgba(251,191,36,0.2)", transform: `scale(${1 + ripple1 * 1.5})`, opacity: (1 - ripple1) * 0.6 }} />
            <div style={{ position: "absolute", width: 140, height: 140, borderRadius: "50%", border: "2px solid rgba(239,68,68,0.15)", transform: `scale(${1 + ripple2 * 2})`, opacity: (1 - ripple2) * 0.5 }} />
            <div style={{ position: "absolute", width: 140, height: 140, borderRadius: "50%", border: "1px solid rgba(251,191,36,0.1)", transform: `scale(${1 + ripple3 * 2.5})`, opacity: (1 - ripple3) * 0.3 }} />
            <div style={{ transform: `scale(${warnScale}) rotate(${warnRotate}deg)`, opacity: Math.min(warnScale, 1) }}>
              <svg width="70" height="70" viewBox="0 0 90 90" style={{ filter: "drop-shadow(0 0 30px rgba(251,191,36,0.8))" }}>
          ...
          <div style={{ fontSize: 48, fontFamily: "'DM Sans',sans-serif", fontWeight: 600, color: "white", textAlign: "center", lineHeight: 1.3, transform: `translateX(${probX}px) translateY(${probY2}px) skewX(${probSkew}deg)`, maxWidth: 1050 }}>
          ...
          <div style={{ position: "relative", opacity: statReveal, transform: `scale(${statZoom * statPulse})` }}>
            <div style={{ position: "absolute", inset: -15, borderRadius: "50%", border: "2px solid rgba(251,191,36,0.3)", transform: `scale(${statPulse})` }} />
            <div style={{ fontSize: 68, fontFamily: "'Epilogue',sans-serif", fontWeight: 800, color: "#fbbf24", textShadow: "0 0 60px rgba(251,191,36,0.7)" }}>65%</div>
          </div>
          <div style={{ transform: `translateX(${sub1X}px)`, opacity: sub1Reveal, fontSize: 32, fontFamily: "'DM Sans',sans-serif", fontWeight: 500, color: "rgba(255,255,255,0.88)", textAlign: "center", maxWidth: 900 }}>
            of patients feel <span style={{ fontWeight: 700, color: "#fff" }}>confused and unsupported</span>
          </div>
          <div style={{ transform: `translateX(${sub2X}px)`, opacity: sub2Reveal, fontSize: 21, fontFamily: "'DM Sans',sans-serif", fontWeight: 400, color: "rgba(255,255,255,0.55)", fontStyle: "italic", textAlign: "center" }}>
            Leading to preventable readmissions and slower recovery
          </div>
          <div style={{ transform: `translateX(${statTicker2X}px)`, opacity: statTicker2, display: "flex", gap: 22 }}>
            {[{ val: "$26B", label: "annual cost of preventable readmissions", color: "239,68,68" }, { val: "30 days", label: "highest-risk window post-discharge", color: "251,191,36" }].map((s, i) => (
              <div key={i} style={{ padding: "9px 20px", backgroundColor: `rgba(${s.color},0.1)`, border: `1px solid rgba(${s.color},0.3)`, borderRadius: 14, textAlign: "center" }}>
                <div style={{ fontSize: 26, fontWeight: 800, color: `rgba(${s.color},1)`, fontFamily: "'Epilogue',sans-serif" }}>{s.val}</div>
                <div style={{ fontSize: 13, color: "rgba(255,255,255,0.55)", fontFamily: "'DM Sans',sans-serif", maxWidth: 200 }}>{s.label}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      {frame >= scene3Start - 25 && frame < scene3Start + 5 && (<><div style={{ position: "absolute", top: 0, bottom: 0, left: 0, width: `${t2sliceP * 50}%`, background: "linear-gradient(180deg,rgba(16,185,129,0.4),rgba(59,130,246,0.3))", opacity: 0.6 }} /><div style={{ position: "absolute", top: 0, bottom: 0, right: 0, width: `${t2sliceP * 50}%`, background: "linear-gradient(180deg,rgba(59,130,246,0.3),rgba(16,185,129,0.4))", opacity: 0.6 }} /></>)}
      ...
      {frame >= scene3Start && frame < scene4Start && (
        <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "row", alignItems: "center", opacity: s3opacity, padding: "50px 110px", gap: 90 }}>
          <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 28 }}>
            <div style={{ transform: `translateY(${s3LogoY}px)`, opacity: s3LogoOpacity }}><div style={{ fontSize: 68, fontFamily: "'Epilogue',sans-serif", fontWeight: 800, background: "linear-gradient(135deg,#10b981,#3b82f6)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", backgroundClip: "text", letterSpacing: "-0.02em", filter: "drop-shadow(0 0 30px rgba(16,185,129,0.6))" }}>Adaptly</div></div>
            <div style={{ fontSize: 44, fontFamily: "'Epilogue',sans-serif", fontWeight: 700, color: "#10b981", transform: `translateY(${headY3}px)`, opacity: headOpacity3, textShadow: "0 0 35px rgba(16,185,129,0.5)", lineHeight: 1.25 }}>Guides them<br />home, safely</div>
            {[
          ...
          <div style={{ flexShrink: 0, transform: `translateY(${phoneY + phoneSway}px) rotate(${phoneRotate}deg) scale(${phoneScale3})`, opacity: phoneOpacity }}>
            <div style={{ width: phoneW, height: phoneH, borderRadius: 50, background: "linear-gradient(145deg,#2a2a2a 0%,#1a1a1a 40%,#111 100%)", padding: 4, boxShadow: `0 0 0 1px rgba(255,255,255,0.12), 0 30px 80px rgba(0,0,0,0.8), 0 0 ${60 * phoneGlow}px rgba(16,185,129,${0.4 * phoneGlow}), inset 0 1px 0 rgba(255,255,255,0.15)`, position: "relative" }}>
              ...
              <div style={{ width: "100%", height: "100%", borderRadius: 47, overflow: "hidden", backgroundColor: "#0d1520", position: "relative" }}>
                ...
                <div style={{ position: "absolute", top: 55, left: 0, right: 0, bottom: 0, padding: "10px 14px" }}>
                  ...
                  {[{ task: "Morning exercises", done: true, time: "8:00 AM" }, { task: "Medication reminder", done: true, time: "10:00 AM" }, { task: "PT Check-in call", done: false, time: "2:00 PM" }].map((t, i) => (
                    <div key={i} style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8, padding: "7px 10px", borderRadius: 10, backgroundColor: t.done ? "rgba(16,185,129,0.08)" : "rgba(255,255,255,0.04)", border: `1px solid ${t.done ? "rgba(16,185,129,0.2)" : "rgba(255,255,255,0.08)"}` }}>
                      <div style={{ width: 16, height: 16, borderRadius: "50%", backgroundColor: t.done ? "#10b981" : "transparent", border: `2px solid ${t.done ? "#10b981" : "rgba(255,255,255,0.2)"}`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>{t.done && <div style={{ width: 6, height: 6, borderRadius: "50%", backgroundColor: "white" }} />}</div>
                      <div style={{ flex: 1 }}>
                ...
                <div style={{ position: "absolute", top: 12, left: 8, right: 8, backgroundColor: "rgba(20,25,40,0.95)", borderRadius: 16, padding: "10px 12px", border: "1px solid rgba(59,130,246,0.4)", transform: `scale(${notifScale})`, opacity: notifOpacity, boxShadow: "0 8px 30px rgba(0,0,0,0.5)", zIndex: 20 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    ...
                    <div><div style={{ fontSize: 10, fontWeight: 700, color: "white", fontFamily: "system-ui" }}>Dr. Martinez</div><div style={{ fontSize: 9, color: "rgba(255,255,255,0.6)", fontFamily: "system-ui" }}>Your PT notes look great today!</div></div>
                  </div>
                </div>
      ...
      {frame >= scene4Start - 20 && frame < scene4Start && (<div style={{ position: "absolute", inset: 0, display: "flex", justifyContent: "center", alignItems: "center", pointerEvents: "none" }}><div style={{ width: 100, height: 100, borderRadius: "50%", background: "radial-gradient(circle,rgba(59,130,246,0.8),rgba(16,185,129,0.4))", transform: `scale(${t3burstS})`, opacity: Math.max(0, t3burstO * (1 - (t3burstS - 1) / 7)) }} /></div>)}
      ...
      {frame >= scene4Start && frame < scene5Start && (
        <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center", opacity: s4opacity, padding: 100, gap: 45 }}>
          ...
          <div style={{ display: "flex", gap: 45, marginTop: 15 }}>
            {[{ emoji: "🏥", label: "Hospital\nApproved", color: "16,185,129", s: b1scale, o: b1opacity }, { emoji: "🔒", label: "HIPAA\nCompliant", color: "99,102,241", s: b2scale, o: b2opacity }, { emoji: "✅", label: "FDA\nCleared", color: "59,130,246", s: b3scale, o: b3opacity }].map((b, i) => (
      ...
      {frame >= scene5Start - 25 && frame < scene5Start + 5 && (<div style={{ position: "absolute", inset: 0, background: "linear-gradient(135deg,rgba(16,185,129,0.35) 0%,rgba(59,130,246,0.25) 100%)", clipPath: `polygon(0 0,${t4diagW * 100}% 0,${t4diagW * 100 - 30}% 100%,0 100%)`, opacity: 0.8 }} />)}

      {/* SCENE 5 — STORIES: reduced padding+gap, smaller card text to prevent cutoff */}
      {frame >= scene5Start && frame < scene6Start && (
        <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center", opacity: s5opacity, padding: "35px 65px", gap: 18 }}>
          <div style={{ overflow: "hidden", opacity: headOpacity5 }}>
            <div style={{ fontSize: 50, fontFamily: "'Epilogue',sans-serif", fontWeight: 700, color: "white", textAlign: "center", clipPath: `inset(0 ${100 - headW5}% 0 0)` }}>Real impact on real people</div>
          </div>
          <div style={{ display: "flex", gap: 20, width: "100%", maxWidth: 1350, justifyContent: "center" }}>
            {[
              { card: { x: card1x, rotY: card1RotY, o: card1Opacity, bg: "16,185,129" }, icon: "❤️", label: "Patient Perspective", labelColor: "#10b981", quote: '"Finally felt supported at home. Clear milestones made recovery less overwhelming."', attr: "— Sarah, recovering from surgery" },
              { card: { x: card2x, rotY: card2RotY, o: card2Opacity, bg: "59,130,246" }, icon: "👨‍⚕️", label: "Care Team", labelColor: "#3b82f6", quote: '"Real-time updates keep our team aligned. We catch issues before they become readmissions."', attr: "— Dr. Martinez, Physical Medicine" },
              { card: { x: card3x, rotY: -card2RotY, o: card3opacity, bg: "99,102,241" }, icon: "🏠", label: "Family Caregiver", labelColor: "#818cf8", quote: '"Knowing exactly what my dad needed each day gave our whole family peace of mind."', attr: "— James, family caregiver" },
            ].map((c, i) => (
              <div key={i} style={{ flex: 1, backgroundColor: `rgba(${c.card.bg},0.09)`, padding: "20px 22px", borderRadius: 22, border: `2px solid rgba(${c.card.bg},0.32)`, transform: `translateX(${c.card.x}px) rotateY(${c.card.rotY}deg)`, opacity: c.card.o, boxShadow: `0 12px 50px rgba(${c.card.bg},0.22)` }}>
                <div style={{ width: 48, height: 48, borderRadius: "50%", backgroundColor: `rgba(${c.card.bg},0.2)`, border: `2px solid rgba(${c.card.bg},0.45)`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 26, marginBottom: 12 }}>{c.icon}</div>
                <div style={{ fontSize: 14, fontFamily: "'DM Sans',sans-serif", fontWeight: 600, color: c.labelColor, marginBottom: 8, letterSpacing: "0.06em", textTransform: "uppercase" }}>{c.label}</div>
                <div style={{ fontSize: 20, fontFamily: "'DM Sans',sans-serif", fontWeight: 500, color: "rgba(255,255,255,0.92)", lineHeight: 1.5, fontStyle: "italic" }}>{c.quote}</div>
                <div style={{ fontSize: 14, fontFamily: "'DM Sans',sans-serif", color: "rgba(255,255,255,0.55)", marginTop: 10 }}>{c.attr}</div>
              </div>
            ))}
          </div>
          <div style={{ fontSize: 26, fontFamily: "'DM Sans',sans-serif", fontWeight: 500, color: "rgba(255,255,255,0.65)", textAlign: "center", transform: `translateY(${bottomY5}px)`, opacity: bottomOpacity5 }}>Better coordination. Better outcomes.</div>
        </div>
      )}
      ...
      {frame >= scene6Start && frame < scene7Start && (
        <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center", opacity: s6opacity, padding: "60px 100px", gap: 45 }}>
          <div style={{ transform: `translateY(${s6HeadY}px)`, opacity: s6HeadOpacity }}><div style={{ fontSize: 62, fontFamily: "'Epilogue',sans-serif", fontWeight: 700, color: "white", textAlign: "center" }}>The results speak <span style={{ color: "#10b981" }}>for themselves</span></div></div>
          <div style={{ display: "flex", gap: 35, width: "100%", maxWidth: 1200, justifyContent: "center" }}>
            {[{ value: stat1Val, scale: stat1Scale, suffix: "%", label: "Reduction in Readmissions", color: "16,185,129", decimals: 0 }, { value: stat2Val, scale: stat2Scale, suffix: "%", label: "Patient Satisfaction Score", color: "59,130,246", decimals: 0 }, { value: stat3Val, scale: stat3Scale, suffix: "x", label: "Faster Care Coordination", color: "99,102,241", decimals: 1 }].map((s, i) => (
              <div key={i} style={{ flex: 1, backgroundColor: `rgba(${s.color},0.08)`, border: `2px solid rgba(${s.color},0.25)`, borderRadius: 24, padding: "32px 24px", display: "flex", flexDirection: "column", alignItems: "center", gap: 10, transform: `scale(${s.scale})`, boxShadow: `0 12px 50px rgba(${s.color},0.15)` }}>
                <div style={{ fontSize: 70, fontFamily: "'Epilogue',sans-serif", fontWeight: 800, color: `rgba(${s.color},1)`, textShadow: `0 0 50px rgba(${s.color},0.6)` }}>{s.decimals === 1 ? s.value.toFixed(1) : Math.round(s.value)}{s.suffix}</div>
          ...
          <div style={{ display: "flex", alignItems: "flex-end", gap: 14, height: 110, padding: "0 20px" }}>
            {[{ h: bar1H, color: "16,185,129", label: "Jan" }, { h: bar2H, color: "59,130,246", label: "Feb" }, { h: bar3H, color: "16,185,129", label: "Mar" }, { h: bar4H, color: "59,130,246", label: "Apr" }, { h: bar5H, color: "16,185,129", label: "May" }].map((b, i) => (
      ...
      {frame >= scene7Start - 20 && frame < scene7Start + 5 && (<div style={{ position: "absolute", top: 0, bottom: 0, left: 0, width: `${t6wipe * 100}%`, background: "linear-gradient(90deg,rgba(16,185,129,0.3),rgba(59,130,246,0.2))", opacity: 0.7 }} />)}
      ...
      {frame >= scene7Start && frame < scene8Start && (
        <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center", opacity: s7opacity, padding: "60px 120px", gap: 55 }}>
          <div style={{ transform: `translateY(${s7HeadY}px)`, opacity: s7HeadO }}><div style={{ fontSize: 60, fontFamily: "'Epilogue',sans-serif", fontWeight: 700, color: "white", textAlign: "center" }}>How <span style={{ color: "#10b981" }}>Adaptly</span> works</div></div>
          <div style={{ display: "flex", alignItems: "center", gap: 0, width: "100%", maxWidth: 1200 }}>
            {[{ s: step1S, o: step1O, icon: "🏥", color: "16,185,129", bg: "#10b981", num: "1", title: "Hospital Discharge", desc: "Care team creates a personalized recovery plan" }, { s: step2S, o: step2O, icon: "📱", color: "59,130,246", bg: "#3b82f6", num: "2", title: "Patient Follows Along", desc: "AI-guided tasks, reminders, and milestone tracking" }, { s: step3S, o: step3O, icon: "✅", color: "16,185,129", bg: "#10b981", num: "3", title: "Team Stays Informed", desc: "Real-time alerts and dashboard updates for the whole team" }].map((st, i) => (
              <React.Fragment key={i}>
                <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 18, transform: `scale(${st.s})`, opacity: st.o }}>
                  <div style={{ width: 100, height: 100, borderRadius: "50%", background: `linear-gradient(135deg,rgba(${st.color},0.25),rgba(${st.color},0.05))`, border: `3px solid rgba(${st.color},0.6)`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 44, boxShadow: `0 0 40px rgba(${st.color},0.3)`, position: "relative" }}>
                    {st.icon}
                    <div style={{ position: "absolute", top: -10, right: -10, width: 28, height: 28, borderRadius: "50%", backgroundColor: st.bg, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14, fontWeight: 800, color: "white", fontFamily: "'Epilogue',sans-serif" }}>{st.num}</div>
                  </div>
                  <div style={{ fontSize: 26, fontFamily: "'DM Sans',sans-serif", fontWeight: 700, color: `rgba(${st.color},1)`, textAlign: "center" }}>{st.title}</div>
                  <div style={{ fontSize: 18, fontFamily: "'DM Sans',sans-serif", color: "rgba(255,255,255,0.6)", textAlign: "center", maxWidth: 280, lineHeight: 1.5 }}>{st.desc}</div>
                </div>
                {i < 2 && (
                  <div style={{ flexShrink: 0, width: 100, display: "flex", alignItems: "center", justifyContent: "center", marginTop: -40, position: "relative" }}>
                    <div style={{ height: 3, width: `${(i === 0 ? connector1W : connector2W) * 100}%`, background: i === 0 ? "linear-gradient(90deg,#10b981,#3b82f6)" : "linear-gradient(90deg,#3b82f6,#10b981)", borderRadius: 2, boxShadow: `0 0 15px rgba(${i === 0 ? "16,185,129" : "59,130,246"},0.5)` }} />
                    {(i === 0 ? connector1W : connector2W) > 0.1 && (
                      <div style={{ position: "absolute", left: `${(i === 0 ? dotPos1 : dotPos2) * (i === 0 ? connector1W : connector2W) * 95}%`, top: "50%", transform: "translate(-50%,-50%)", width: 12, height: 12, borderRadius: "50%", backgroundColor: "white", boxShadow: `0 0 15px rgba(${i === 0 ? "16,185,129" : "59,130,246"},0.9)`, marginTop: -20 }} />
                    )}
                  </div>
                )}
              </React.Fragment>
      ...
      {frame >= scene8Start - 10 && frame < scene8Start + 10 && (<div style={{ position: "absolute", inset: 0, background: "linear-gradient(135deg,rgba(99,102,241,0.5),rgba(16,185,129,0.4))", opacity: t7flash }} />)}
        ...
        <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center", opacity: s8opacity, gap: 45 }}>
          ...
          <div style={{ position: "relative", width: 580, height: 380 }}>
            ...
            {[
              ...
              const r = 165; const x = 290 + r * Math.cos(rad); const y = 190 + r * Math.sin(rad);
              return (
      ...
      {frame >= scene9Start - 20 && frame < scene9Start + 5 && (<div style={{ position: "absolute", inset: 0, background: "radial-gradient(circle at center, rgba(16,185,129,0.4), rgba(59,130,246,0.2))", clipPath: `circle(${t8wipe * 80}% at 50% 50%)`, opacity: 0.7 }} />)}
        ...
        <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center", opacity: s9opacity, padding: "60px 100px", gap: 55 }}>
          ...
          <div style={{ position: "relative", width: "100%", maxWidth: 1200, height: 240 }}>
            <div style={{ position: "absolute", top: 55, left: 80, right: 80, height: 3, backgroundColor: "rgba(255,255,255,0.1)", borderRadius: 2 }}>
              <div style={{ height: "100%", width: `${timelineW * 100}%`, background: "linear-gradient(90deg,#10b981,#3b82f6,#10b981)", borderRadius: 2, boxShadow: "0 0 20px rgba(16,185,129,0.5)" }} />
            </div>
            {[{ label: "Discharged", sublabel: "Day 0", emoji: "🏥", color: "16,185,129", s: j1S, o: j1O, pos: "10%" }, { label: "First Check-in", sublabel: "Day 3", emoji: "📋", color: "59,130,246", s: j2S, o: j2O, pos: "38%" }, { label: "Milestone Met", sublabel: "Day 14", emoji: "🎯", color: "99,102,241", s: j3S, o: j3O, pos: "65%" }, { label: "Full Recovery", sublabel: "Day 30", emoji: "⭐", color: "251,191,36", s: j4S, o: j4O, pos: "90%" }].map((j, i) => (
      ...
      {frame >= scene10Start - 15 && frame < scene10Start + 10 && (<div style={{ position: "absolute", inset: 0, background: "linear-gradient(135deg,rgba(16,185,129,0.4),rgba(99,102,241,0.3))", opacity: t9flash }} />)}
            ...
            <div style={{ backgroundColor: "rgba(15,20,35,0.9)", borderRadius: 24, border: "1px solid rgba(59,130,246,0.2)", overflow: "hidden", boxShadow: "0 25px 80px rgba(0,0,0,0.6), 0 0 50px rgba(59,130,246,0.1)" }}>
              <div style={{ padding: "18px 24px", backgroundColor: "rgba(59,130,246,0.08)", borderBottom: "1px solid rgba(59,130,246,0.15)", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                ...
                <div style={{ display: "flex", gap: 8 }}>
                  {["🟢 Active", "⚠️ Alert", "📋 All"].map((tag, i) => (<div key={i} style={{ padding: "4px 12px", borderRadius: 20, backgroundColor: i === 1 ? "rgba(239,68,68,0.15)" : "rgba(255,255,255,0.05)", border: `1px solid ${i === 1 ? "rgba(239,68,68,0.4)" : "rgba(255,255,255,0.1)"}`, fontSize: 12, color: "rgba(255,255,255,0.7)", fontFamily: "'DM Sans',sans-serif" }}>{tag}</div>))}
                </div>
              </div>
      ...
      {frame >= scene11Start - 15 && frame < scene11Start + 10 && (<div style={{ position: "absolute", inset: 0, background: "radial-gradient(ellipse at center, rgba(16,185,129,0.5), rgba(59,130,246,0.3), transparent)", opacity: t10flash }} />)}

      {/* SCENE 11 — CTA */}
      {frame >= scene11Start && (
        <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center", opacity: s11opacity, gap: 38 }}>
          {[300, 500, 700].map((size, i) => (<div key={i} style={{ position: "absolute", width: size, height: size, borderRadius: "50%", border: `1px solid rgba(16,185,129,${0.08 - i * 0.02})`, transform: `rotate(${orbitAngle * (i % 2 === 0 ? 0.3 : -0.2)}deg)`, pointerEvents: "none" }} />))}
          <div style={{ transform: `scale(${heartScale11 * heartbeat})` }}>
            <svg width="110" height="110" viewBox="0 0 100 100" style={{ filter: `drop-shadow(0 0 ${35 * glowPulse11}px rgba(16,185,129,${glowPulse11}))` }}>
          ...
          <div style={{ fontSize: 70, fontFamily: "'Epilogue',sans-serif", fontWeight: 700, color: "white", textAlign: "center", transform: `translateY(${headline11Y}px)`, opacity: headline11Opacity, lineHeight: 1.25, maxWidth: 1100 }}>Better outcomes.<br /><span style={{ color: "#10b981" }}>Healthier patients.</span></div>
          <div style={{ fontSize: 34, fontFamily: "'DM Sans',sans-serif", fontWeight: 500, color: "rgba(255,255,255,0.78)", textAlign: "center", maxWidth: 900, lineHeight: 1.4, transform: `translateY(${sub11Y}px)`, opacity: sub11Opacity }}>From hospital to home, we're with them every step</div>
          <div style={{ marginTop: 8, padding: "22px 65px", fontSize: 36, fontFamily: "'Epilogue',sans-serif", fontWeight: 700, color: "#10b981", border: "2px solid #10b981", borderRadius: 18, transform: `scale(${cta11Scale})`, opacity: cta11Opacity, boxShadow: `0 0 60px rgba(16,185,129,${urlGlow * 0.35}),inset 0 0 40px rgba(16,185,129,${0.06 * urlGlow})`, backgroundColor: "rgba(16,185,129,0.1)", letterSpacing: "-0.01em" }}>adaptlyapp.com</div>
          <div style={{ display: "flex", gap: 30, opacity: tag11Opacity }}>
            {["🏥 Hospital-Ready", "🔒 HIPAA Secure", "📱 iOS & Android"].map((tag, i) => (<div key={i} style={{ padding: "8px 20px", backgroundColor: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.12)", borderRadius: 30, fontSize: 18, color: "rgba(255,255,255,0.6)", fontFamily: "'DM Sans',sans-serif" }}>{tag}</div>))}
          </div>
          <div style={{ fontSize: 20, fontFamily: "'DM Sans',sans-serif", color: "rgba(255,255,255,0.38)", fontWeight: 400, textAlign: "center", opacity: tag11Opacity }}>Empowering recovery through intelligent care coordination</div>
        </div>
      )}
    </div>
  );
