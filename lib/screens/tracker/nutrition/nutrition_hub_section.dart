...
typedef LogFoodCallback = Future<void> Function(FoodEntry food, MealType mealType);
...
class NutritionHubCard extends StatelessWidget {
  ...
  const NutritionHubCard({super.key, required this.conditions, required this.onLogFood});
          ...
          Row(
            children: [
              ...
              FilledButton.tonalIcon(
                ...
                icon: Icon(Icons.arrow_forward, size: 18, color: cs.onPrimaryContainer),
                label: Text('Open', style: text.labelLarge?.semiBold.withColor(cs.onPrimaryContainer)),
              ),
            ],
          ),
  ...
  void _openHub(BuildContext context) {
    showModalBottomSheet<void>(
      ...
      builder: (_) => NutritionHubSheet(conditions: conditions, onLogFood: onLogFood),
    );
  }
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            SizedBox(height: 4),
            Text(food.name, maxLines: 1, overflow: TextOverflow.ellipsis, style: text.titleSmall?.semiBold),
            SizedBox(height: 2),
            Text('${food.caloriesPerServing} kcal • ${food.proteinG.toStringAsFixed(0)}P',
                style: text.labelSmall?.withColor(cs.onSurfaceVariant)),
            const Spacer(),
            if (food.tags.isNotEmpty)
              Text('#${food.tags.first}', maxLines: 1, overflow: TextOverflow.ellipsis, style: text.labelSmall?.withColor(cs.primary)),
          ],
        ),
      ),
    );
...
class NutritionHubSheet extends StatefulWidget {
  final List<String> conditions;
  final LogFoodCallback onLogFood;
  const NutritionHubSheet({super.key, required this.conditions, required this.onLogFood});

  @override
  State<NutritionHubSheet> createState() => _NutritionHubSheetState();
}

class _NutritionHubSheetState extends State<NutritionHubSheet> with SingleTickerProviderStateMixin {
          ...
          child: Column(
            children: [
              Padding(
                ...
                child: Container(width: 38, height: 4, decoration: BoxDecoration(color: cs.outlineVariant, borderRadius: BorderRadius.circular(99))),
              ),
              Padding(
                padding: AppSpacing.paddingLg.copyWith(top: 0, bottom: AppSpacing.sm),
                child: Row(
                  children: [
                    Container(
                      width: 36, height: 36,
                      decoration: BoxDecoration(color: cs.primaryContainer.withValues(alpha: 0.5), borderRadius: BorderRadius.circular(12)),
                      child: Icon(Icons.restaurant_menu, color: cs.onPrimaryContainer, size: 20),
                    ),
                    ...
                    Expanded(
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Text('Nutrition Hub', style: text.titleLarge?.semiBold),
                          Text('NIH & MedlinePlus-backed food intelligence',
                              style: text.labelSmall?.withColor(cs.onSurfaceVariant)),
                        ],
                      ),
                    ),
                    IconButton(onPressed: context.pop, icon: Icon(Icons.close, color: cs.onSurfaceVariant)),
                  ],
                ),
              ),
              Padding(
                ...
                child: TextField(
                  ...
                  decoration: InputDecoration(
                    hintText: 'Search foods, recipes, conditions, nutrients…',
              ...
              Expanded(
                child: TabBarView(
                  ...
                  children: [
                    ...
                    _RecipesTab(recipes: recipes, scrollController: scrollController),
                    _PlansTab(plans: hub.plans, scrollController: scrollController),
                  ],
                ),
              ),
            ],
      ...
      children: [
        if (conditions.isNotEmpty) ...[
          Container(
            ...
            child: Row(
              children: [
                Icon(Icons.health_and_safety_outlined, size: 18, color: cs.onPrimaryContainer),
                SizedBox(width: AppSpacing.sm),
                Expanded(
                  child: Text(
                    'Curated for: ${conditions.take(4).join(', ')}${conditions.length > 4 ? ' +${conditions.length - 4}' : ''}',
                    style: text.labelSmall?.semiBold.withColor(cs.onPrimaryContainer),
                  ),
                ),
              ],
            ),
        ...
        GridView.builder(
          ...
          itemBuilder: (context, i) => _FoodTile(food: foods[i], onTap: () => onOpenFood(foods[i])),
        ),
        SizedBox(height: AppSpacing.lg),
        _SectionHeader(icon: Icons.menu_book, title: 'Recipes from NIH & MedlinePlus'),
        SizedBox(height: AppSpacing.sm),
        ...
        SizedBox(height: AppSpacing.lg),
        _SectionHeader(icon: Icons.tips_and_updates_outlined, title: 'Eating plans worth knowing'),
        SizedBox(height: AppSpacing.sm),
        ...
        Expanded(
          child: foods.isEmpty
              ? Center(
                  child: Padding(
                    padding: AppSpacing.paddingLg,
                    child: Text('No foods match your search yet.', style: text.bodyMedium?.withColor(cs.onSurfaceVariant)),
                  ),
                )
              : GridView.builder(
                  controller: scrollController,
  ...
  Widget build(BuildContext context) {
    ...
    return InkWell(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: selected ? cs.primary.withValues(alpha: 0.15) : cs.surfaceContainerHighest.withValues(alpha: 0.45),
          borderRadius: BorderRadius.circular(99),
          border: Border.all(color: selected ? cs.primary.withValues(alpha: 0.5) : cs.outlineVariant.withValues(alpha: 0.35)),
        ),
        child: Row(
          ...
          children: [
            ...
            SizedBox(width: 6),
            Text(label, style: (selected ? text.labelMedium?.semiBold.withColor(cs.primary) : text.labelMedium?.withColor(cs.onSurface))),
          ],
        ),
      ),
    );
  ...
  Widget build(BuildContext context) {
    ...
    if (recipes.isEmpty) {
      return Center(
        child: Padding(
          padding: AppSpacing.paddingLg,
          child: Text('No recipes match your search yet.', style: text.bodyMedium?.withColor(cs.onSurfaceVariant)),
        ),
      );
    }
    return ListView.separated(
      ...
      itemBuilder: (context, i) => _RecipeRow(recipe: recipes[i], onTap: () => _open(context, recipes[i].url)),
    );
  }
...
class _PlansTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return ListView.separated(
      ...
      separatorBuilder: (_, __) => SizedBox(height: AppSpacing.sm),
      itemBuilder: (context, i) => _PlanRow(plan: plans[i], onTap: () => _open(context, plans[i].url)),
    );
  }
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(food.name, maxLines: 1, overflow: TextOverflow.ellipsis, style: text.titleSmall?.semiBold),
            SizedBox(height: 2),
            Text('${food.caloriesPerServing} kcal • ${food.proteinG.toStringAsFixed(0)}P ${food.carbsG.toStringAsFixed(0)}C ${food.fatG.toStringAsFixed(0)}F',
                style: text.labelSmall?.withColor(cs.onSurfaceVariant), maxLines: 1, overflow: TextOverflow.ellipsis),
            const Spacer(),
            if (food.tags.isNotEmpty)
              Wrap(
                spacing: 4,
                runSpacing: 4,
                children: food.tags.take(2).map((t) => Container(
                  padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 2),
                  decoration: BoxDecoration(
                    color: cs.primary.withValues(alpha: 0.10),
                    borderRadius: BorderRadius.circular(99),
                  ),
                  child: Text(t, style: text.labelSmall?.withColor(cs.primary)),
      ...
      child: Container(
        ...
        child: Row(
          children: [
            Container(
              width: 44, height: 44,
              decoration: BoxDecoration(color: cs.secondaryContainer.withValues(alpha: 0.45), borderRadius: BorderRadius.circular(12)),
            ...
            Expanded(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text(recipe.title, style: text.titleSmall?.semiBold, maxLines: 1, overflow: TextOverflow.ellipsis),
                  SizedBox(height: 2),
                  Text(recipe.description, style: text.bodySmall?.withColor(cs.onSurfaceVariant), maxLines: 2, overflow: TextOverflow.ellipsis),
                  SizedBox(height: 4),
                  Text('${recipe.sourceName} • ${recipe.estMinutes} min', style: text.labelSmall?.withColor(cs.primary)),
                ],
              ),
            ),
          ...
          children: [
            Row(
              children: [
                Container(
                  width: 44, height: 44,
                  decoration: BoxDecoration(color: cs.tertiaryContainer.withValues(alpha: 0.5), borderRadius: BorderRadius.circular(12)),
                  child: Icon(Icons.bookmark_outline, color: cs.onTertiaryContainer),
                ),
                SizedBox(width: AppSpacing.md),
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(plan.tagline, style: text.labelSmall?.withColor(cs.primary)),
                    ],
                  ),
                ),
                Icon(Icons.open_in_new, size: 18, color: cs.onSurfaceVariant),
            ...
            SizedBox(height: AppSpacing.sm),
            Text(plan.description, style: text.bodySmall?.withColor(cs.onSurfaceVariant)),
            SizedBox(height: AppSpacing.sm),
            Wrap(
              spacing: 6,
              runSpacing: 6,
              children: plan.goodFor.take(4).map((g) => Container(
                padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4),
                decoration: BoxDecoration(
                  color: cs.primary.withValues(alpha: 0.1),
                  borderRadius: BorderRadius.circular(99),
                ),
                child: Text(g, style: text.labelSmall?.withColor(cs.primary)),
              )).toList(growable: false),
            ),
            SizedBox(height: AppSpacing.sm),
            Text('Emphasize: ${plan.emphasize.take(5).join(', ')}', style: text.labelSmall?.withColor(cs.onSurfaceVariant)),
            SizedBox(height: 2),
            Text('Limit: ${plan.limit.take(4).join(', ')}', style: text.labelSmall?.withColor(cs.onSurfaceVariant)),
            SizedBox(height: 4),
            Text(plan.sourceName, style: text.labelSmall?.withColor(cs.onSurfaceVariant)),
          ],
        ),
      ),
    );
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  SizedBox(height: 2),
                  Text(article.summary, style: text.bodySmall?.withColor(cs.onSurfaceVariant), maxLines: 2, overflow: TextOverflow.ellipsis),
                  SizedBox(height: 4),
                  Text('${article.sourceName} • ${article.estReadMinutes} min read', style: text.labelSmall?.withColor(cs.primary)),
                ],
              ),
            ),
            Icon(Icons.open_in_new, size: 18, color: cs.onSurfaceVariant),
          ...
          child: SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          ...
                          Text('${food.category.label} • ${food.servingLabel}', style: text.labelSmall?.withColor(cs.onSurfaceVariant)),
                        ],
                      ),
                    ),
                    IconButton(onPressed: context.pop, icon: Icon(Icons.close, color: cs.onSurfaceVariant)),
                ...
                SizedBox(height: AppSpacing.md),
                Row(children: [
                  macro('Calories', '${food.caloriesPerServing}', Icons.local_fire_department_outlined),
                  SizedBox(width: AppSpacing.sm),
                  macro('Protein', '${food.proteinG.toStringAsFixed(1)}g', Icons.fitness_center_outlined),
                ]),
                SizedBox(height: AppSpacing.sm),
                Row(children: [
                  macro('Carbs', '${food.carbsG.toStringAsFixed(1)}g', Icons.grain_outlined),
                  SizedBox(width: AppSpacing.sm),
                  macro('Fat', '${food.fatG.toStringAsFixed(1)}g', Icons.opacity_outlined),
                  SizedBox(width: AppSpacing.sm),
                  macro('Fiber', '${food.fiberG.toStringAsFixed(1)}g', Icons.eco_outlined),
                ]),
                SizedBox(height: AppSpacing.lg),
                if (food.healthBenefits.isNotEmpty) ...[
                  Text('Why it\'s good for you', style: text.titleSmall?.semiBold),
                  SizedBox(height: AppSpacing.sm),
                  for (final b in food.healthBenefits) Padding(
                ...
                if (food.goodForConditions.isNotEmpty) ...[
                  ...
                  Wrap(
                    ...
                    children: food.goodForConditions.map((c) => Container(
                      padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 6),
                      decoration: BoxDecoration(
                        color: cs.primary.withValues(alpha: 0.1),
                        borderRadius: BorderRadius.circular(99),
                      ),
                      child: Text(c, style: text.labelSmall?.withColor(cs.primary)),
                    )).toList(growable: false),
                  ),
                  SizedBox(height: AppSpacing.md),
                ],
                if (food.note != null) ...[
                  Container(
                    ...
                    child: Row(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Icon(Icons.info_outline, color: cs.onErrorContainer, size: 18),
                        SizedBox(width: AppSpacing.sm),
                        Expanded(child: Text(food.note!, style: text.bodySmall?.withColor(cs.onErrorContainer))),
                      ],
                    ),
                  ),
                  SizedBox(height: AppSpacing.md),
                ...
                SizedBox(height: AppSpacing.md),
                Center(
                  child: Text('Data informed by NIH MyPlate & MedlinePlus guidelines.',
                      style: text.labelSmall?.withColor(cs.onSurfaceVariant)),
                ),
              ],
            ),
