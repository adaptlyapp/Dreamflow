...
/// Family portal version of the therapist screen.
/// Shows notes and resources shared by the patient's care team.
class FamilyTherapistScreen extends StatefulWidget {
  const FamilyTherapistScreen({super.key});

  @override
  State<FamilyTherapistScreen> createState() => _FamilyTherapistScreenState();
}

class _FamilyTherapistScreenState extends State<FamilyTherapistScreen>
  ...
  Future<void> _loadData() async {
    ...
    final familyUserId = context.read<UserProvider>().currentUser?.id;
    ...
    try {
      ...
      final connection = await _familyService.getPrimaryConnection(familyUserId);
      if (connection == null) {
        debugPrint('FamilyTherapistScreen: No active patient connection');
        if (mounted) setState(() => _isLoading = false);
        return;
      }

      final patientUserId = connection.patientId;
      
      // Load notes and resources for the patient
      final results = await Future.wait([
        _therapistService.getNotesForPatient(patientUserId),
        _therapistService.getResourcesForPatient(patientUserId),
        _therapistService.getPatientSummary(patientUserId),
      ]);
      ...
      debugPrint('FamilyTherapistScreen._loadData error: $e');
      if (mounted) setState(() => _isLoading = false);
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text('${_patientName ?? 'Patient'}\'s Care Team'),
        leading: IconButton(
        ...
        Expanded(
          child: _filteredNotes.isEmpty
              ? _buildEmptyState(
                  ...
                  title: 'No notes yet',
                  subtitle: 'Notes from the care team will appear here',
                )
              : ListView.builder(
        ...
        Expanded(
          child: _filteredResources.isEmpty
              ? _buildEmptyState(
                  ...
                  subtitle: 'Resources shared by the care team will appear here',
                )
              : ListView.builder(
                  padding: EdgeInsets.all(AppSpacing.md),
