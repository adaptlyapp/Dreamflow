...
class _PlanEditorScreenState extends State<PlanEditorScreen> {
  ...
  Future<void> _load() async {
    ...
    List<Milestone> list = [];
    try {
      list = await _service.list(userId: userId, conditionId: widget.conditionId);
    } catch (e, st) {
    ...
    try {
      timelines = await _timelineService.list(userId: userId, conditionId: widget.conditionId);
    } catch (e, st) {
      timelineError = true;
  ...
  Future<void> _showEducationFor(Milestone m) async {
    ...
    try {
      ...
      final detailJson = conditionDetails[widget.conditionId];
      if (detailJson != null) {
  ...
  Future<bool> _saveGeneratedPlan(List<Map<String, dynamic>> plan, {required int durationDays}) async {
    ...
    // Replace existing milestones for this condition with freshly generated ones
    try {
      await _service.replaceForCondition(userId: userId, conditionId: widget.conditionId, items: newItems);
      debugPrint('[PlanEditor] Replaced ${_items.length} existing items with ${newItems.length} new items');
      _sessionChanged = true;
      _sessionSavedToCloud = true;
      _pendingSnapshotSync = true;
      return true;
  ...
  Future<void> _sharePlan() async {
    ...
    try {
      final now = DateTime.now();
      final related = <String>{widget.conditionId, ...user.conditions};
      final post = Post(
        id: const Uuid().v4(),
        authorId: user.id,
  ...
  Widget build(BuildContext context) {
    ...
    return WillPopScope(
      onWillPop: () async {
        // Return a result to previous screen so it can show confirmation
        if (Navigator.of(context).canPop()) {
          context.pop({'changed': _sessionChanged, 'saved': _sessionSavedToCloud});
          return false;
        }
        return true;
      },
      child: Scaffold(
          appBar: AppBar(
            ...
            title: Text('Plan • ${widget.conditionName}', maxLines: 1, overflow: TextOverflow.ellipsis),
            actions: [
              IconButton(
