...
--
-- Your remote deploy runner is applying a migration named exactly:
--   20260915194830_dreamflow_generated.sql
--
...
--
-- Root cause:
-- Some linked Supabase projects already had a legacy `public.resources_curated`
-- table that predates the provider-directory schema. Earlier migrations used
-- `CREATE TABLE IF NOT EXISTS`, which does NOT add missing columns on an existing
-- table. SQL-language functions validate referenced columns at CREATE time, so we
-- must ensure all referenced columns exist *before* creating the RPC.

-- 1) Safety-net schema drift fix -------------------------------------------------
-- Guarded so this migration is safe on brand-new databases where the table might
-- not yet exist at the moment this runs.
DO $$
BEGIN
