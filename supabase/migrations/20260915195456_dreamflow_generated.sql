-- Dreamflow generated migration (override / compat patch)
--
-- Your Supabase deploy logs (Sep 15, 2026) show it is applying a migration named:
--   20260915195456_dreamflow_generated.sql
-- and failing while compiling:
--   public.search_provider_directory(...)
-- with:
--   ERROR: column s.city does not exist (SQLSTATE 42703)
--
-- This happens because LANGUAGE sql functions are validated at CREATE time.
-- If the remote database has a legacy `public.resources_curated` table (created
-- via CREATE TABLE IF NOT EXISTS in older migrations), missing columns like
-- `city` are NOT added automatically.
--
-- Fix: ensure all referenced columns exist BEFORE creating/replacing the RPC.

-- 0) No-op fixes seen in the deploy log ------------------------------------------
DO $$
BEGIN
  IF to_regclass('public.resources_curated') IS NOT NULL THEN
    -- Some legacy DBs are missing this column; on yours it already exists.
    ALTER TABLE public.resources_curated ADD COLUMN IF NOT EXISTS specialty TEXT;
  END IF;
END$$;

-- The log shows: "index uq_resources_curated_source does not exist, skipping".
-- Keep it safe/idempotent here too.
...
AS $$
  ...
  SELECT
    ...
    s.phone, s.contact_email, s.website, s.hours,
    s.availability, s.description, s.services_offered, s.service_area,
    s.service_kinds, s.capabilities, s.populations_served, s.conditions_served,
    s.insurance_accepted,
    s.does_repairs, s.does_rentals, s.does_sales,
