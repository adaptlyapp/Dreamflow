-- Dreamflow generated migration (compat patch)
--
-- Your deploy runner is attempting to apply a migration named exactly:
--   20260915195257_dreamflow_generated.sql
--
-- It fails on legacy databases with:
--   ERROR: column s.city does not exist (SQLSTATE 42703)
-- while compiling `public.search_provider_directory()`.
--
-- Postgres validates referenced columns at CREATE time for LANGUAGE sql functions,
-- so we must ensure `public.resources_curated` has the provider-directory columns
-- *before* creating/replacing the RPC.
